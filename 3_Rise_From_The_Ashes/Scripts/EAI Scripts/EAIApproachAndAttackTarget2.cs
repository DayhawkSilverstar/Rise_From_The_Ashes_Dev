...
public class EAIApproachAndAttackTarget2 : EAIBase
{
    ...
    private List<TargetClass> targetClasses;
    ...
    private IconicZombie iconicZombie;
    ...
    public override void Init(EntityAlive _theEntity)
    {
        ...
        MutexBits = 3;
        executeDelay = 0.1f;
        iconicZombie = _theEntity as IconicZombie;
        Log.Out("EAIApproachAndAttackTarget2 - Init : IconicZombie - " + iconicZombie.GetType().ToString());
    }
    ...
    {
        Log.Out("EAIApproachAndAttackTarget2 - CanExecute");
        ...
        {
            Log.Out("EAIApproachAndAttackTarget2 - EntityTarget is null");
            return false;
        }
        ...
        for (int i = 0; i < targetClasses.Count; i++)
        {
            ...
            if (targetClass.type != null && targetClass.type.IsAssignableFrom(type))
            {
                ...
                Log.Out("EAIApproachAndAttackTarget2 - Chase Time " + chaseTimeMax.ToString());
                return true;
            }
        }

        Log.Out("EAIApproachAndAttackTarget2 - EntityTarget is not valid");
        return false;
    }

    public override void Start()
    {
        Log.Out("EAIApproachAndAttackTarget2 - Start");
    ...
    {
        Log.Out("EAIApproachAndAttackTarget2 - Continue");
        if (theEntity.sleepingOrWakingUp || theEntity.bodyDamage.CurrentStun != 0)
        {
            Log.Out("EAIApproachAndAttackTarget2 - attack target = false");
            return false;
        }
        ...
        if (isGoingHome)
        {
            if (!attackTarget)
            {
                Log.Out("EAIApproachAndAttackTarget2 - going home and attack target = false");
        ...
        {
            Log.Out("EAIApproachAndAttackTarget2 - target is dead so eat");
            return false;
        }
    ...
    {
        Log.Out("EAIApproachAndAttackTarget2 - Reset");        
    ...
    {
        Log.Out("EAIApproachAndAttackTarget2 - Update");
        ...
        if (!flag)
        {
            if (theEntity.navigator.noPathAndNotPlanningOne() && pathCounter > 0 && num5 < 2.1f)
            {
                ...
                iconicZombie.SetMoveTo(moveToLocation2, _canBreakBlocks: true);
            }
        }
        else
        {
