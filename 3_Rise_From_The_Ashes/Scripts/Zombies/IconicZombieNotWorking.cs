...
public class IconicZombie : EntityZombie
{
    ...
    // Cached tunables
    private float meleeReachAdjust = -1.3f;      // default to current behavior
    private float verticalXZWindow = 1.5f;       // used by tasks, cached here for consistency
    private float verticalOffsetThreshold = 0.5f;
    private int stopChaseTicksLOSLoss = 100;
    private int breakModeCooldownTicks = 40;

    // Properties to expose cached tunables
    public float MeleeReachAdjust => meleeReachAdjust;
    public float VerticalXZWindow => verticalXZWindow;
    public float VerticalOffsetThreshold => verticalOffsetThreshold;
    public int StopChaseTicksLOSLoss => stopChaseTicksLOSLoss;
    public int BreakModeCooldownTicks => breakModeCooldownTicks;

    // Persistent vertical attempt tracking (shared across tasks)
    public float VerticalTryTimerSec { get; set; }
    public float VerticalTryTargetSec { get; set; }
    public int VerticalTimerTargetId { get; set; } = -1;
    public bool ForceBreakRequest { get; set; }
    ...
    public override void Init(int _entityClass)
    {
        ...
        // Read CVars once during Init, fallback to default constants
        meleeReachAdjust = GetCVarOrDefault("iconic_melee_reach_adjust", -1.3f);
        verticalXZWindow = GetCVarOrDefault("iconic_vertical_xz_window", 1.5f);
        verticalOffsetThreshold = GetCVarOrDefault("iconic_vertical_offset_threshold", 0.5f);
        stopChaseTicksLOSLoss = (int)GetCVarOrDefault("iconic_stop_chase_ticks_los", 100f);
        breakModeCooldownTicks = (int)GetCVarOrDefault("iconic_break_mode_ticks", 40f);

        // Initialize vertical attempt tracking lazily (do not reset on task switches)
        if (VerticalTryTargetSec <= 0f)
        {
            VerticalTryTargetSec = 10f + UnityEngine.Random.Range(0f, 5f);
        }
    ...
    public bool InMeleeRange(Entity target)
    {
        ...
        {
            distance -= targetCollider.bounds.extents.magnitude;
        }
        ...
        if (distance < ZombieReach + meleeReachAdjust)
        {
#if DEBUG
            // Log the adjusted position for debugging purposes
    ...
    // Force walking move toward position (no sprint/charge). Uses same pathing adjustments as SetMoveTo.
    public void MoveWalkTo(Vector3 pos)
    {
#if DEBUG
        Log.Out("Iconic Zombie - MoveWalkTo " + pos.ToString());
#endif
        // Flatten Y if target above/below
        if (TargetAbove() || TargetBelow())
        {
            pos = new Vector3(pos.x, position.y, pos.z);
        }

        lastPosition = this.position;

        // Explicitly force walk locomotion; clear any sprint/charge flags
        MovementRunning = false;
        Raging = false;
        SpecialAttack = false;
        SpecialAttack2 = false;

        // Drive the move using the same helper and allow block break consideration on the path
        this.moveHelper.SetMoveTo(pos, true);
    }
    ...
    public void SetMoveTo(Vector3 _pos, bool _canBreakBlocks)
    {
        ...
        if (TargetAbove() || TargetBelow()) // short-circuit logical op
        {
        ...
        foreach (EntityAlive entityAlive in list)
        {
            ...
            if (!(entityAlive is EntityDrone) && check(entityAlive))
            {
                ...
                if (distance < closeTargetDist)
                {
                    ...
                    closeTargetDist = distance;
                    Target = entityAlive;
                    investigatePos = entityAlive.position;
                }
    ...
    protected bool check(EntityAlive entity)
    {
        ...
        try
        {
            ...
            if (player != null && !CanSeeStealth(GetDistance(player), player.Stealth.lightLevel))
            {
                return false;
            }
        }
        ...
        {
#if DEBUG
            Log.Out("IconicZombie - check exception: " + ex.Message);
#endif
            return false;
        }
    ...
    public bool TargetXZCheck()
    {
        ...
        // Check if the target is within verticalXZWindow units in the X and Z axes
        if (Math.Abs(this.position.x - this.Target.position.x) < verticalXZWindow && 
            Math.Abs(this.position.z - this.Target.position.z) < verticalXZWindow)
        {
            return true; // Return true if the target is within the specified distance
        }
        
        return false; // Return false if the target is not within the specified distance
    }
    ...
    public bool TargetAbove()
    {
        ...
        // Check if the target is above by more than the vertical offset threshold
        // NOTE: We do NOT check TargetXZCheck() here - that should be done separately in AI logic
        // This method only determines if there's a vertical displacement upward
        if (this.Target.position.y - verticalOffsetThreshold > this.position.y)
        {
#if DEBUG
            // Log the adjusted position for debugging purposes
    ...
    public bool TargetBelow()
    {
        ...
        // Check if the target is below by more than the vertical offset threshold
        // NOTE: We do NOT check TargetXZCheck() here - that should be done separately in AI logic
        // This method only determines if there's a vertical displacement downward
        return this.Target.position.y + verticalOffsetThreshold < this.position.y;
    }
    ...
    public void SeekNoise(EntityPlayer player)
    {
        ...
        if (playerTargetClassIndex >= 0 && targetClasses != null)
        {
    ...
    private float GetCVarOrDefault(string name, float defVal)
    {
        try
        {
            float v = GetCVar(name);
            if (v == 0f && !HasCVarSet(name))
            {
                return defVal;
            }
            return v;
    ...
    private bool HasCVarSet(string name)
    {
        // No direct has-API; consider non-zero as set.
        return GetCVar(name) != 0f;
    }
}
